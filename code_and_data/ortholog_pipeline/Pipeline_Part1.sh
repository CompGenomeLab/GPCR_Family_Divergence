...
mkdir $protein"_"$date"_"$class
cd $protein"_"$date"_"$class

echo ============= BLAST ==============
module load blast-plus-2.9.0-gcc-9.2.0-k73hoo5
database="/cta/users/bselcuk/GPCRA/database/canonical_blast_database/uniprot_reference_proteomes_canonical_db"
blastp -query "/cta/users/bselcuk/GPCRA/GPCR_atlas_26-10-2022/"$class"_fasta/"$protein"_HUMAN.fasta" -db $database -max_target_seqs 10000 -evalue 100000 -num_threads 10 -outfmt "6 qseqid sseqid pident length mismatch gapopen send evalue bitscore stitle" -out $protein"_result_tabular.txt"
echo "BLAST is done!"

echo ============= Obtaining FASTA files to form tree ==============
module load py-pip-19.3-gcc-9.2.0-tfsp3uc
python ../Scripts/FastaObtainerV2.py --blastout $protein"_result_tabular.txt" --num 3 --taxid 9606 --out $protein"_blast.fasta" >> $protein"_"$date"_pipeline.log"
...
fftns --thread -1  $protein"_blast.fasta" > $protein"_MSA"$date".fasta" 

echo =======Alignment Trimming ===========
module load python-3.8.0-gcc-9.2.0-gjpyy5a
clipkit $protein"_MSA"$date".fasta" -m kpic-gappy -o $protein"_MSA"$date"_kpicgappy.fasta"
echo "Alignment file is produced in FASTA format."

echo =======Fast Tree Running =================
echo "Using fasttree for tree construction."
module load fasttree-2.1.10-gcc-9.2.0-b5pgbi6
FastTreeMP $protein"_MSA"$date"_kpicgappy.fasta" > $protein"_blasttree_"$date

echo ===== Obtaining gene clade and sequences =====
module load py-pip-19.3-gcc-5.4.0-goul4og
module load ete-3.1.1

directory="/cta/users/bselcuk/GPCRA/GPCR_atlas_26-10-2022/"
python $directory"Scripts/gene_clade_find.py" --tree $protein"_blasttree_"$date --blastout $protein"_result_tabular.txt" --out $protein"_"$date"_subtree.nwk" 
python $directory"Scripts/subtree_fasta.py" --tree $protein"_"$date"_subtree.nwk" --out $protein"_"$date"_subtree.fasta"

echo ==== Producing representative sequneces file ====
module load cdhit-4.8.1-gcc-9.2.0-yp5pkad 

cd-hit -i $protein"_"$date"_subtree.fasta" -o $protein"_"$date"_subtree_reps_0.7.fasta" -c 0.7

cd-hit -i $protein"_"$date"_subtree.fasta" -o $protein"_"$date"_subtree_reps_0.65.fasta" -c 0.65
