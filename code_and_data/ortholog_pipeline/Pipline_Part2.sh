...
#SBATCH --output=%j-GPCR-Pipeline_Part2.out
...
#protein_array=(GP143 GP157)

protein_array=(TPRA1)

protein=${protein_array[$SLURM_ARRAY_TASK_ID]}
date=26-10-2022
cd $protein"_"$date"_classU"

directory="/cta/users/bselcuk/GPCRA/GPCR_atlas_26-10-2022/"

echo ===== Second MSA =====
module load mafft-7.407-gcc-9.2.0-fww6zrx
linsi --thread -1 --maxiterate 1000 $protein"_"$date"_subtree.fasta" > $protein"_subtreeMSA"$date".fasta"  

echo ===== Model selection ====
module load iq-tree-2.0.6-gcc-9.2.0-fufn5ou
iqtree2 -s $protein"_subtreeMSA"$date".fasta" -m MF -mfreq FO -cmin 2 -cmax 15  -T AUTO --mset JTT --mrate R #--redo

python $directory"Scripts/iqtree_model_get.py" --logfile $protein"_subtreeMSA"$date".fasta.log" --modelout $protein"_bestmodel.txt"
model=$(head -n 1 $protein"_bestmodel.txt")

echo ===== Second Tree =====
module load raxml-ng-1.0.3
raxml-ng --threads auto{10} --tree pars{10},rand{10} --search --workers auto --data-type AA --model $model --force msa_dups --force msa_names --seed 2 -msa $protein"_subtreeMSA"$date".fasta"  --prefix $protein"_subtree_"$date #--bs-trees autoMRE{500} --bs-metric tbe,fbp
...
python $directory"Scripts/ete3trimmer.py" --tree $protein"_subtree_"$date".raxml.bestTree" --protein $protein --fasta $protein"_blast.fasta" --out $protein"_subtree_"$date"_orthologs.nwk" >> $protein"_"$date"_pipeline.log"
python $directory"Scripts/ete3order.py" --tree $protein"_subtree_"$date"_orthologs.nwk" --out $protein"_subtree_"$date"_orthologs_ordered.nwk" >> $protein"_"$date"_pipeline.log"
python $directory"Scripts/subtree_fasta.py" --tree $protein"_subtree_"$date"_orthologs_ordered.nwk"  --out $protein"_"$date"_orthologs_ordered.fasta" >> $protein"_"$date"_pipeline.log"
